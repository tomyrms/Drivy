...
| **Planning AP31–AP43, AP100/101, commercial-terms** | **Aucun.** Les leçons des tests capture sont insérées en SQL direct (`INSERT INTO drivy.lesson`). `g2-lessons.md` le reconnaît : « aucun parcours HTTP nominal… déclaré passé ». |
| **AP45–AP48, AP53, AP55, AP58, AP65** | **Aucun test automatisé.** `g2-reports.md` cite un contrôle HTTP manuel local, non reproductible dans le dépôt. |
...
| **Permis / documents** | ✘ (le Lesson renvoie `permitWarning:true` en dur) | ✘ | ✘ | **AP29/AP30 contrôle de permis**, AP59–AP64/AP90 documents (upload, scellement, ticket, suppression), stockage objet |
...
| **Leçons / constat / annulation / frais** | AP39–AP43 ✔ **sans test**. AP49 ✔T (partiel). AP65 ✔ sans test | Agenda, planifier, déplacer, annuler, constat | ✘ | **AP44 absence (no-show)**, **AP50/AP88 correction de constat**, calcul des frais d’annulation (non implémenté : AP43 ne crée aucune charge), constat forcé avec `anomalyReason` tant qu’AP30 manque |
...
| **Bilans / progression** | AP52–AP54, AP56 ✔T. AP53, AP55, AP58, AP45–AP48 ✔ **sans test** | Préparation, bilan, bilans publiés, progression | ✘ | **AP57 retrait de publication**, AP51 brouillon de correction par un autre auteur, pièces jointes (dépend de F09) |
...
**P0-1 · Filet de tests du planning et du bilan déjà codés** (S–M)
...
**P0-3 · Contrôle de permis AP29/AP30** (M)
...
**P0-4 · Issues réelles d’une leçon : absence, correction, retrait** (M)
...
**P0-5 · Exploitation minimale avant données réelles** (M)
...
|---|---|---|---|
| 1 | P0-1 tests planning/bilan + CI sur push | — | Filet avant toute modification de `lessons.ts`/`lesson-reports.ts` |
| 2 | P0-2 SMTP et worker déployés | web (acceptation), admin | Sans eux, aucune personne nouvelle n’entre ; le code existe déjà |
| 3 | P0-5 santé, sauvegarde, restauration | exploitation | Des comptes réels existent déjà |
| 4 | P0-3 permis AP29/AP30 | web admin, badge iOS moniteur | Supprime l’anomalie systématique au constat |
| 5 | P0-4 AP44, puis AP50/AP88, puis AP57 | iOS moniteur, lecture élève | Boucle leçon honnête : absence, erreur, retrait |
