...
def run(chromium=None):
 ...
 with sync_playwright() as pw:
  ...
  scene('capture');p.evaluate('capture="recording";render();clearTimeout(toastTimer);document.getElementById("toast").style.display="none"');p.set_viewport_size({'width':390,'height':1000});p.screenshot(path=str(R/'annexes/live-capture-v3-13.png'),full_page=True)
  click('editLive');p.screenshot(path=str(R/'annexes/live-form-v3-13.png'),full_page=True);p.keyboard.press('Escape')
  scene('bilan');p.screenshot(path=str(R/'annexes/live-bilan-v3-13.png'),full_page=True)
 ...
 return {'version':'3.13','scope':'DOCUMENTARY_HTML_MEMORY_AND_DTO_ONLY','checks':checks,'dtoCases':dtos,'passed':not errors,'errors':errors,'nativeTestsExecuted':False,'limits':['No persistence, GPS, backend, SQL transaction or publication executed.','No user test, driving safety test or native Dynamic Type/accessibility validation.','Scenarios T407–T422 remain NOT_EXECUTED on a product.']}
if __name__=='__main__':
 ...
 if a.write_report:(R/'annexes/verification-live-prototype-v3-13.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
