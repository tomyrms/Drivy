...
def main(args):
    ...
    ck('Reader title V3.17','V3.17' in soup.title.text)
    ...
    current=soup.select_one('#doc-06-gouvernance-audit-corrections-v3-17');old=soup.select_one('#doc-06-gouvernance-audit-corrections-v3-16')
    ...
    return {'version':'3.17','scope':'READER_AND_COMPLEMENTARY_GALLERY_SMOKE_ONLY','passed':all(x['passed'] for x in checks),'errorCount':sum(not x['passed'] for x in checks),'checks':checks,'environment':{'python':platform.python_version(),'browser':version},'previousFullGalleryInteractions':'NOT_RERUN','nativeTestsExecuted':False}
if __name__=='__main__':
    ...
    if a.write_report:(R/'annexes/verification-lecteur-v3-17.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
