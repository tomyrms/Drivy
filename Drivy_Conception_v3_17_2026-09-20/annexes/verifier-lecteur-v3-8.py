...
def run(chromium=None):
    ...
    sections=soup.select('section.document');check('73_markdown_documents',len(sections)==73 and len(list(R.rglob('*.md')))==73)
    ...
    with sync_playwright() as p:
        ...
        check('all_documents_navigated',len(visited)==73)
        page.fill('#search','Cartographie native');page.wait_for_timeout(100)
        visible=page.locator('.navlink:visible').count();check('search_returns_relevant_subset',0<visible<73,visible)
        ...
        for width in [1440,834,390]:
            ...
            page.screenshot(path=str(R/f'annexes/lecteur-v3-8-{width}.png'),full_page=False)
...
if __name__=='__main__':
    ...
    if a.write_report:(R/'annexes/verification-lecteur-v3-8.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n', encoding="utf-8")
