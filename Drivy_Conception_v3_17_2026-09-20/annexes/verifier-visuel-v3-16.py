...
def run():
    ...
    provenance=json.loads((R/'annexes/provenance-v3-16.json').read_text(encoding='utf-8'))
    ...
    for tag,obj in [('__FIXTURE__',json.loads((a/'fixture.json').read_text(encoding='utf-8'))),('__TOKENS_JSON__',tokens),('__MAP_JSON__',(a/'map.svg').read_text(encoding='utf-8')),('__DENSE_MAP_JSON__',(a/'map-dense.svg').read_text(encoding='utf-8'))]:
    ...
    return {'version':'3.16','scope':'STRUCTURAL_DOCUMENTARY_CHECKS_ONLY','passed':all(x['passed'] for x in checks),'errorCount':sum(not x['passed'] for x in checks),'checks':checks,'markdownDocuments':count,'figmaImportExecuted':False,'nativeTestsExecuted':False}
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--write-report',action='store_true');a=p.parse_args();r=run()
    if a.write_report:(R/'annexes/verification-visuel-v3-16.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
