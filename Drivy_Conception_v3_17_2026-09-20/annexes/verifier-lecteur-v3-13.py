...
def run(chromium=None):
    ...
    sections=soup.select('section.document');check('80_markdown_documents',len(sections)==80 and len(list(R.rglob('*.md')))==80)
    ...
    with sync_playwright() as p:
        ...
        check('all_documents_navigated',len(visited)==80)
        page.fill('#search','Cartographie native');page.wait_for_timeout(100)
        visible=page.locator('.navlink:visible').count();check('search_returns_relevant_subset',0<visible<80,visible)
        ...
        for width in [1440,834,390]:
            ...
            page.screenshot(path=str(R/f'annexes/lecteur-v3-13-{width}.png'),full_page=False)
...
if __name__=='__main__':
    ...
    if a.write_report:(R/'annexes/verification-lecteur-v3-13.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n', encoding="utf-8")
