...
final class SchoolCaptureSessionController {
    ...
    var canResume: Bool { state == .paused && context?.lease.permitsCollection() == true }
    ...
    func adoptAndStart(transfer: SchoolCaptureTransferCoordinator, source: any SchoolCaptureLocationProviding,
        ...
        guard permittedScope == transfer.scope, session.scope == transfer.scope,
        ...
        let clock = try SchoolCaptureClockReference(serverTime: authorization.serverTime, receivedAt: receivedAt)
        ...
        do {
            ...
            guard generation == request, permittedScope == active.scope, state == .preparing else { throw SchoolCaptureStorageFailure.closed }
            ...
            guard generation == request, state == .preparing else { throw error }
    ...
    func resume() async {
        ...
        do {
            ...
            guard generation == request, state == .preparing, current.serverCapture.captureState == .authorized else { return }
    ...
    func finalize(allowPartial: Bool) async {
        guard let active = context, state == .saved, !isTransferring else { return }
        let request = generation
        isTransferring = true; transferMessage = nil
        defer { if generation == request { isTransferring = false } }
        do {
    ...
    private func remoteStop(captureID id: UUID?, request: UUID) {
        guard generation == request, let active = context, id == nil || id == active.session.id,
              state != .saved, state != .stopping, state != .failed else { return }
        let boundary = active.stopBoundary()
    ...
    private func failCollector(_ active: Context, request: UUID, error: Error) {
        ...
        let boundary = active.stopBoundary()
        active.local.halt(); state = .stopping; errorMessage = message(error)
        Task { await finish(active, request: request, boundary: boundary, reason: .deviceError) }
    }
